...
import { toggleDrawer, handleSelectedItem } from '../../actions';
 ...
 * @param {*} props.selectedItem the currently selected item to display in
...
const PresentationListItems = (props) => {
    const { selectedItem, onClick, menuItemTitles } = props;
        ...
        <Fragment>
            {menuItemTitles.map((title) =>
                <ListItem button onClick={() => onClick(title)} key={title}
                    sx={{
                        backgroundColor: selectedItem === title ? PRIMARY_COLOR : 'inherit',
                        '&:hover': {
...
const findSelectedComponent = (selectedItem, test) => {
    const component = [
...
function LeftBannerDrawer(props) {
    const { open, selectedItem, dispatch } = props;
                ...
                >
                    ...
                    <List>
                        <PresentationListItems
                            ...
                            onClick={(title) => dispatch(handleSelectedItem(title))}
...
export default function MainDrawer(props) {
    const { title, user, logoutAction } = props;

    const [state, dispatch] = useReducer(reducers, undefined, initialState);
    const { open, selectedItem } = state;
    ...
    return (
        <Box sx={{ width: '100%', height: '90%', display: 'flex' }} >
            ...
            <LeftBannerDrawer open={open}
                selectedItem={selectedItem} dispatch={dispatch}
            />

            <MainContent open={open} >
                <DrawerHeader />
                <Box sx={{ width: '100%', height: '100%' }} >
                    {findSelectedComponent(selectedItem, { ...state, dispatch }).component}
                </Box>
            </MainContent>
